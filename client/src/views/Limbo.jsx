...
const Limbo = (user, isAuthenticated) => {
  ...
  const betProcessing = useRef(false);
  const betTimeout = useRef(null);
  ...
  const handleLimboBet = () => {
    if (!betProcessing.current) {
      setLoading(true);
      betProcessing.current = true;
      limboSocket.emit("limbo:bet", { amount: betAmount, multiplier: multiplier });

      betTimeout.current = setTimeout(() => {
        setLoading(false);
        betProcessing.current = false;
      }, 1500);
    }
  };
  ...
  useEffect(() => {
    ...
    limboSocket.on("limbo:result", (result) => {
      setPreviousRolls((prevRolls) => [...prevRolls, result.crashpoint]);
      setLimboResult(result);
    });
    return () => {
      limboSocket.off("game-creation-error", creationError);
      limboSocket.off("limbo:bet", success);
      limboSocket.off("limbo:result", open);
      clearTimeout(betTimeout.current);
    };
  }, [addToast]);
...
>
  {loading ? (
    ...
    'Place Bet'
          ...
          <div className={classes.previousRolls} style={{ position: 'absolute', top: "10px", right: "10px", fontFamily: "Poppins", fontSize: "12px", color: "#FFF", display: "flex", alignItems: "center", justifyContent: "space-between", flexDirection: "row" }}>
            {previousRolls.slice(-5).map((roll, index) => (
              <span key={index} style={{ marginRight: '5px', color: roll > 1 ? "rgb(61, 209, 121)" : "rgb(241, 50, 62)" }}>{roll.toFixed(2)}x</span>
            ))}
          </div>
