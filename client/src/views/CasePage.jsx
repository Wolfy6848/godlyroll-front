...
import Gold from "../assets/colors/green.png";
...
import GoldBG from "../assets/colors/greenbg.png";
...
import GoldBlob from "../assets/colors/GREENY.png";
...
const CasePage = () => {
  ...
  const getBackgroundForItem = (item) => {
    ...
    switch (item.color) {
        ...
        case "red":
            ...
            break;
        case "gold":
            background = "radial-gradient(50% 50% at 50% 50%, rgb(144, 238, 144) 0%, rgba(0, 0, 0, 0) 100%)";
            break;
        default:
            background = ""; 
    }
  ...
  async function spin(data) {
    ...
    for (const box of w) {
      ...
      numberElement.textContent = `R$${number}`;
  ...
  const debouncedButtonClick = useDebounce(openCase, 1000);
  ...
  const getColorSet = (color) => {
    switch(color) {
      ...
      case "gold":
        return { Color: Gold, Background: GoldBG, Hex: "#90EE90", Blob: GoldBlob }
      default:
        return { Color: Grey, Background: GreyBG, Hex: "#91A0B1", Blob: GreyBlob }
    }
  };
        ...
        <div className={classes.lootTableContainer}>
          {caseData.items.map((item, index) => {
            ...
            >
              ...
              <a style={{fontSize: 10, textAlign: "center", width: "80%", wordBreak: "wrap", color: "#FFFFFF", fontWeight: 500, marginTop: 20}}>{item.name}</a>
