            ...
            <Collapse key={index}>
              <Box key={player.betId}>
                <Box className={classes.bet}>
                  ...
                  <Box ml="auto" style={{ color: redResult ? "green" : "red" }}>{greenResult || blackResult ? "-" : ""}<img src={coin} style={{ width: 12, height: 12}}/> {redResult ? (player.betAmount * multiplier).toFixed(2) : player.betAmount.toFixed(2)}</Box>
                </Box>
              </Box>
            </Collapse>
          ))}
