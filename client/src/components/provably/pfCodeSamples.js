...
console.table({ GAME_ID, isHashValid: PRIVATE_HASH === hash, module });
  `,
  limbo: `const Chance = require("chance");
const crypto = require("crypto");

// Game information
const GAME_ID = ""; // Game ID, also sometimes referred to as Round ID
const PRIVATE_SEED = ""; // Private seed, revealed after round has ended
const PRIVATE_HASH = ""; // Private hash, revealed before round has ended
const PUBLIC_SEED = ""; // Public seed, randomly generated from EOS Blockchain

// Generate a hash from the private seed
const hash = crypto.createHash("sha256").update(PRIVATE_SEED).digest("hex");

// Construct a new chance instance
const chance = new Chance(PRIVATE_SEED + "-" + GAME_ID + "-" + PUBLIC_SEED);

// Generate a random, repeatable module to determine the crash point (float between 1 and 1000)
const crashPoint = chance.floating({ min: 1.01, max: 1000, fixed: 2 });

console.table({ GAME_ID, isHashValid: PRIVATE_HASH === hash, crashPoint });  
    `,
  dice: `const crypto = require("crypto");

const GAME_ID = ""; // Example Game ID
const PRIVATE_SEED = ""; // Private seed, revealed after the round has ended
const PRIVATE_HASH = ""; // Private hash, revealed before the round has ended
const CLIENT_SEED = ""; // Client seed, provided by the client
...
// Verify the private hash matches the hash of the private seed
const isHashValid = PRIVATE_HASH === sha256(PRIVATE_SEED);

// Generate a game hash using HMAC-SHA256 with the private seed and client seed
const gameHash = crypto.createHmac("sha256", PRIVATE_SEED).update(CLIENT_SEED).digest("hex");

// Function to derive a number from the game hash
function getNumberFromHash(hash) {
    return parseInt(hash.slice(0, 13), 16); // Using first 13 hex characters for a large number
}

// Generate a random roll (number from 0 to 99.9999) using the game hash
const roll = (getNumberFromHash(gameHash) % 1000000) / 10000;

console.table({ GAME_ID, isHashValid, roll });
    `,
  mines: `const Chance = require("chance");
const crypto = require("crypto");
...
// Generate a random, repeatable module to determine dice roll result (number from 1-100)
const roll = chance.floating({ min: 1, max: 100, fixed: 2 });

console.table({ GAME_ID, isHashValid: PRIVATE_HASH === hash, roll });  
      `,
