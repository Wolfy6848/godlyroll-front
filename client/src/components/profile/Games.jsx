...
import { makeStyles, Modal, Table, TableBody, TableCell, TableHead, TableRow } from "@material-ui/core";
...
const useStyles = makeStyles(theme => ({
  ...
  tran: {
    ...
    "& tr": {
      background: "#14151D",
      borderRadius: "4px",
      borderTop: "8px solid #0e1017",
      borderBottom: "8px solid #0e1017",
    },
    "& th": {
      border: "none",
      color: "#C0C1DE",
...
const Games = ({ userId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const updatedGameData = {
        ...
        limbo: response.limbo.map(game => ({...game, type: "Limbo"})).sort((a, b) => new Date(b.created) - new Date(a.created)),
        dice: response.dice.map(game => ({...game, type: "Dice"})).sort((a, b) => new Date(b.created) - new Date(a.created)),
        mines: response.mines.map(game => ({...game, type: "Mines"})).sort((a, b) => new Date(b.created) - new Date(a.created)),
      };
      ...
      const allGames = [
        ...
        ...updatedGameData.limbo,
        ...updatedGameData.dice,
        ...updatedGameData.mines
      ];
  ...
  useEffect(() => {
    switch (selected) {
      ...
      case "limbo":
        return setCurrentData(gameData.limbo);
      case "dice":
        return setCurrentData(gameData.dice);
      case "mines":
        return setCurrentData(gameData.mines);
    }
  }, [selected])
  ...
  return (
    <div className={classes.root}>
        ...
        <div className={classes.modal}>
          {selectedGameDetails && (
            <div className={classes.modalContent}>
              <h2>Game Details</h2>
              {selectedGameDetails.gameId && <p>Game ID: {selectedGameDetails.gameId}</p>}
              {selectedGameDetails.serverSeed && <p>Server Seed: {selectedGameDetails.serverSeed}</p>}
              {selectedGameDetails.privateSeed && <p>Private Seed: {selectedGameDetails.privateSeed}</p>}
              {selectedGameDetails.gameSeed && <p>Private Seed: {selectedGameDetails.gameSeed}</p>}
              {selectedGameDetails.blockHash && <p>Block Hash: {selectedGameDetails.blockHash}</p>}
              {selectedGameDetails.clientSeed && <p>Client Seed: {selectedGameDetails.clientSeed}</p>}
              {selectedGameDetails.gameHash && <p>Private Seed: {selectedGameDetails.gameHash}</p>}
              {selectedGameDetails.privateHash && <p>Private Hash: {selectedGameDetails.privateHash}</p>}
              {selectedGameDetails.serverHash && <p>Server Hash: {selectedGameDetails.serverHash}</p>}
            </div>
          )}
        </div>
      </Modal>
      ...
      <div className={classes.selectionContainer}>
        ...
        <div 
          className={selected == "Limbo" ? classes.active : classes.notactive}
          onClick={() => setSelected("limbo")}
        >
          Limbo
        </div>
        <div 
          className={selected == "Dice" ? classes.active : classes.notactive}
          onClick={() => setSelected("dice")}
        >
          Dice
        </div>
        <div 
          className={selected == "Mines" ? classes.active : classes.notactive}
          onClick={() => setSelected("mines")}
        >
          Mines
        </div>
      </div>
              ...
              <TableBody>
                {currentData.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <motion.div whileTap={{ scale: 0.97 }} className={classes.details} onClick={() => handleOpenModal(item)}>Details</motion.div>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
