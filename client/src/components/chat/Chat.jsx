...
const useStyles = makeStyles({
  ...
  switchContainer: {
    position: "fixed",
    bottom: "1.25rem",
    left: props => (props.hidden ? "calc(100% - 40px)" : "calc(100% - 340px)"),
...
const Chat = ({ hidden }) => {
  ...
  const [isMessageVisible, setMessagesVisible] = useState(true);

  const toggleMessages = () => {
    setMessagesVisible(!isMessageVisible);
  };
  ...
  return (
    <Fragment>
      {hidden ? (
        ...
        <>
          <Header giveaway={giveaway} toggleMessages={toggleMessages} isMessageVisible={isMessageVisible} />
          <Messages loading={loading} chatMessages={chatMessages} open={isMessageVisible} />
